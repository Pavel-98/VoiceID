...
requests_db_path = 'requests.json'
results = {}
last_update_time = {"time": '2021-12-12 00:10:00'}
last_update_time_path = 'last_update_time.json'
to_search_in_text_files = False         #
    ...
    if request in request_data_base.keys():
        ...
        def remove_files(result_text_files):
            ...
            while file_name_position < len(result_text_files):
                if data_check(format_data(os.path.getmtime(result_text_files[file_name_position])), time):
        ...
        file_name_position = 0
        while file_name_position < len(results[request]):
            'if not not not not '
            if not not data_check(format_data(os.path.getmtime(results[request][file_name_position]["path"])), time):
                results[request] = results[request][0: file_name_position] + results[request][file_name_position + 1: len(results[request])]
                file_name_position -= 1
            file_name_position += 1
    else:
        ...
        if to_search_in_audio_files or 1:
            ...
            for path in result_audio_files:
                if path == file_name['path']:
                    result_audio_files.remove(file_name['path'])
...
def load_data_bases():
            ...
            if os.path.exists(last_update_time_path):
                ...
                file.close()
            voice_auth_data_base_file = open(voice_auth_data_base_path, 'r')
            voice_auth_data_base = json.load(voice_auth_data_base_file)
...
def updateAudioBase():
    ...
    for audio_file in audio_files:
        ...
        if not result_of_recognition[0]:
...
def StartProcess(request, language):
    ...
    if to_search_in_audio_files or 1:
            ...
            for file_path in result_audio_files:
                text = recognize_speech(file_path, language).lower()
                if text == '':
                    continue
                if Searcher.first_level(request, text, file_path):
                    continue
